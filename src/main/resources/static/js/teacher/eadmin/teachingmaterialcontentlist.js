    ...
    if (taskContent != null && taskContent.length > 0) {
        if (firstId != 0) {
            ...
            for (var i = 0; i < taskContent.length; i++) {
                if (taskContent[i].teachingMaterialHeadId == firstId) {
            ...
            for (var i = 0; i < titleIds.length; i++) {
                ...
                if (titleIds[i] != firstId) {
                    ...
                    for (var j = 0; j < taskContent.length; j++) {
                        if (taskContent[j].teachingMaterialHeadId == titleIds[i]) {
            ...
            for (var k = 0; k < ownTitleIds.length; k++) {
                for (var i = 0; i < cell; i++) {
                    ...
                    for (var j = 0; j < taskContent.length; j++) {
                        if (taskContent[j].teachingMaterialHeadId == ownTitleIds[k] && taskContent[j].contentX == contentX[i]) {//填写所在行
            ...
            //输出操作列
            for (var i = 0; i < cell; i++) {
                var s = '<a href="javascript:;" onclick="toReport(' + contentX[i] + ',' + templateId + ','+teachingMaterialInfo.id+');" >填报</a>';
...
function toReport(x, id,teachingMaterialInfoId) {
    window.location.href = web_path + '/teacher/eadmin/teachingMaterialContentAdd?x=' + x + "&templateId=" + id + "&teachingMaterialInfoId="+teachingMaterialInfoId;
}
