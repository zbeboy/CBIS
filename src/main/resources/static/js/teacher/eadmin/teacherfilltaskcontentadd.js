...
function outputHtml(){
    ...
    for(var i = 0;i<taskContent.length;i++){
        for(var j = 0;j<taskTitle.length;j++){
            if(taskContent[i].teacherFillTaskHeadId == taskTitle[j].id && taskTitle[j].isAssignment == 1){
                ...
                isRight = true;
            } else if(taskContent[i].teacherFillTaskHeadId == taskTitle[j].id && taskTitle[j].isAssignment == 0){
...
function toBack(){
    window.location.href = web_path + "/teacher/eadmin/teacherFillTaskContentList?templateId=" + templateId + "&teacherTaskInfoId="+teacherFillTaskInfo.id;
}
