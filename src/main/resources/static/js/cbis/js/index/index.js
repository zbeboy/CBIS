...
function toMajor(obj) {
    ...
    if ($(p.children()[0]).text().trim().length > 0 && $(p.children()[1]).text().trim().length > 0) {
        window.location.href = "/user/majorArticleShow?majorId=" + $(p.children()[0]).text() + "&id=" + $(p.children()[1]).text();
    }
}
...
function moreMajor(obj) {
    ...
    if (majorId.trim().length > 0 && articleInfoId.trim().length > 0) {
        window.location.href = "/user/majorArticleShow?majorId=" + majorId + "&id=" + articleInfoId;
    }
}
