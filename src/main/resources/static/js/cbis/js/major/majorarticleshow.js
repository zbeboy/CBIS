...
function outputHtml(data) {
    $(target.bigTitle).attr('href', '/user/majorArticleShow?id=' + target.id + "&majorId=" + target.majorId).text(data.articleInfo.bigTitle);
    $(target.realname).text(data.articleInfo.userRealName);
    $(target.date).text(data.articleInfo.date);
    $(target.imgurl).attr('href', '/user/majorArticleShow?id=' + target.id + "&majorId=" + target.majorId);
...
function action() {
    if(target.id != null && target.id > 0){
        $.post("/user/majorArticleShowData", {
            'id': target.id
        }, function (data) {
            outputHtml(data);
        }, 'json');
...
function toMajor(obj) {
    window.location.href = '/user/articleMajorData?navId=' + target.navId + "&majorId=" + $(obj).attr("data");
}