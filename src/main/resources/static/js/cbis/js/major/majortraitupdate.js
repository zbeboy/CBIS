...
$(document).ready(function () {
    ...
    if ($('#imgPath').text() != '') {
        ...
        $('#articleimg').attr('src', '/cbis/files/majortrait/' + str);
...
function updateArticle() {
    ...
    subData.push(new articleData(title, summary, picPath, "", "", "专业特色", -1));
    /*可以没有子标题数据，只组装第一条*/
        ...
        if ($($('.mysubtitle')[i]).val().trim().length > 0 || $($('.mysubpage')[i]).val().trim().length > 0) {/*当子标题与内容都为空时，不算入文章*/
            if ($($('.mysubtitle')[i]).val().trim().length <= 50) {
                if ($($('.mysubpage')[i]).val().trim().length <= 2000) {
                    subData.push(new articleData(title, summary, picPath, repalceHtmlCode($($('.mysubtitle')[i]).val()), repalceHtmlCode($($('.mysubpage')[i]).val()), "专业特色", i));
                } else {
...
function sendArtitle(subData) {
    ...
    $.post('/cbis/maintainer/updatemajortrait', {
        'subData': subData,
        'id': id,
        ...
        if (status) {
            if (data.state) {
                layer.confirm(data.msg, {
                    ...
                    window.location.href = "/cbis/maintainer/majortrait";
                });
            } else {
                layer.msg(data.msg);
            }
...
$(function () {
    var progressbar = $("#progressbar"),
        ...
        settings = {
            ...
            params: {'pathname': 'majortrait'},//json 数据 该文件服务器端相对 files下路径
