...
$(document).ready(function () {
    ...
    if ($('#imgPath').text() != '') {
        ...
        $('#articleimg').attr('src', '/cbis/files/tieelegant/' + str);
...
function deleteArticle() {
    ...
    layer.confirm('你确定要删除该文章吗？', {
        ...
        $.post('/cbis/maintainer/deletetieelegant', {
            ...
            if (status) {
                if (data.state) {
                    layer.msg(data.msg);
                    window.location.href = "/cbis/maintainer/tieelegant";
                } else {
                    layer.msg(data.msg);
                }
            }
        ...
        if ($($('.mysubtitle')[i]).val().trim().length > 0 || $($('.mysubpage')[i]).val().trim().length > 0) {/*当子标题与内容都为空时，不算入文章*/
            if ($($('.mysubtitle')[i]).val().trim().length <= 50) {
                if ($($('.mysubpage')[i]).val().trim().length <= 2000) {
                    subData.push(new articleData(title, summary, picPath, repalceHtmlCode($($('.mysubtitle')[i]).val()), repalceHtmlCode($($('.mysubpage')[i]).val()), "系风采", i));
                } else {
...
function sendArtitle(subData) {
    ...
    $.post('/cbis/maintainer/updateelegant', {
        'subData': subData,
        'id': $('#articleInfoId').text()
    }, function (data, status) {
        layer.close(index);
