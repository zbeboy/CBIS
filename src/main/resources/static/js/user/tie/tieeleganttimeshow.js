/**
 * Created by lenovo on 2016-03-03.
 */
...
function outputHtml(id, elementIndex) {
    $.post(web_path + '/user/tie/tieElegantTimeDropData', {
            'id': id,
            'bigTitle': bigTitle,
                        ...
                        $('<li>').append(
                            $('<div class="uk-grid">').append(
                                $('<div class="uk-width-4-5 uk-text-truncate" >').append(
                                    $('<a>').attr('href', web_path + '/user/tie/tieElegantShow?id=' + data.result[i].id).text(data.result[i].bigTitle)
                                )
                            ).append(
