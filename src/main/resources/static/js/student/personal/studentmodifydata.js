/**
 * Created by lenovo on 2016-05-15.
 */
/**
 * 上传图片到服务器端
 */
...
function initParam() {
    ...
    param.parentName = $('#parentName').val().trim();
    param.parentContactPhone = $('#parentContactPhone').val().trim();
    param.dormitoryNumber = $('#dormitoryNumber').val().trim();
    param.placeOrigin = $('#placeOrigin').val().trim();
    param.problemSituation = $('#problemSituation').val().trim();
...
function validateAll() {
    ...
    if(parentContactPhone.length>0){
        ...
        if (!regex.test(parentContactPhone)) {
            $('#parentContactPhone').removeClass('uk-form-success').addClass('uk-form-danger');
            $('#parentContactPhoneError').text('请输入正确的手机号!');
            $('#parentContactPhone').focus();
            return false;
        } else {
            $('#parentContactPhone').removeClass('uk-form-danger').addClass('uk-form-success');
            $('#parentContactPhoneError').text('');
        }
    }
    ...
    $('#parentName').removeClass('uk-form-danger').addClass('uk-form-success');
    $('#dormitoryNumber').removeClass('uk-form-danger').addClass('uk-form-success');
    $('#placeOrigin').removeClass('uk-form-danger').addClass('uk-form-success');
    $('#problemSituation').removeClass('uk-form-danger').addClass('uk-form-success');
