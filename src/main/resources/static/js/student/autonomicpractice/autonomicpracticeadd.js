        ...
        for (var i = 0; i < heads.length; i++) {
            if (useTitle(heads[i].authority)) {
                ...
                if (heads[i].typeValue === 'database') {//数据库字段
                    ...
                    if (!validateField(heads[i].title, heads[i].typeValue, $(id).val(), heads[i].databaseTableField)) {
                        isRight = false;
                        break;
                    }
                } else {//自定义字段
                    if (heads[i].typeValue === 'checkbox') {
                        if (!validateField(heads[i].title, heads[i].typeValue, $("input[name='" + heads[i].titleVariable + "']:checked"), '')) {
                            isRight = false;
                            break;
                        }
                    } else {
                        var id = "#" + heads[i].titleVariable;
                        if (!validateField(heads[i].title, heads[i].typeValue, $(id).val(), '')) {
                            isRight = false;
                            break;
                        }
                    }
 ...
 * @returns {boolean}
 */
function validateField(title, headType, value, databaseTableField) {
    if (headType === 'text') {
                    ...
                    if (isUpdate) {
                        for (var j = 0; j < myContents.length; j++) {
                            if (heads[i].id == myContents[j].autonomousPracticeHeadId) {
                                updateContent = myContents[j].cotent;
                                break;
                            }
                        }
                    }
...
function showDetailsDialog(client) {
    layer.confirm('去编辑？', {
        ...
        window.location.href = web_path + '/maintainer/major/majorHeadUpdate?majorId=' + client.id;
    });
}
