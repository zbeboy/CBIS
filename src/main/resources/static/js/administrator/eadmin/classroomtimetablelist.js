    ...
    for(var i = 0;i< list.length;i++){
        ...
        $('#tableData').append(
            $('<tr>')
                ...
                .append($('<td>').text(dealNull(list[i].classroom)))
 ...
 */
function toEdit(id){
    window.location.href = web_path + '/administrator/eadmin/classroomTimetableUpdate?id=' + id+'&teachType='+param.teachType;
}
 ...
 */
function toLook(id){
    window.location.href = web_path + '/administrator/eadmin/classroomTimetableLook?id='+id+'&teachType='+param.teachType;
}
 ...
 */
function toDel(id){
    layer.confirm("确定要删除该教室课表吗?", {
        btn: ['确定','取消'] //按钮
    }, function(){
        var index = layer.load(1, {
            shade: [0.1,'#fff'] //0.1透明度的白色背景
        });
        $.post(web_path + '/administrator/eadmin/deleteClassroomTimetable',{
            'id':id
        },function(data){
            layer.close(index);
            if(data.state){
...
function action(){
    $.post(web_path + '/administrator/eadmin/classroomTimetableData',param,
        function(data){
            if(data.state){
