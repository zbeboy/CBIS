    ...
    $.post(web_path + "/maintainer/users/teacherAllData", {
        ...
        if (data.state) {
            ...
            for (var i = 0; i < data.result.length; i++) {
                if(teacher.id == data.result[i].id){
                    $('#teacherData').append(
                        $('<li>').append(
                            $('<label>').append(
 ...
 */
var param = {
    'id': $('#teacherTimetableId').val().trim(),
 ...
 */
function initParam() {
    param.id = $('#teacherTimetableId').val().trim();
    ...
    $('#updateForm').validator({
        ...
        valid: function (form) {
            ...
            if(param.teacherId == undefined || param.teacherId == 0 ){
                ...
                me.holdSubmit();
                $.ajax({
                    url: web_path + "/administrator/eadmin/updateTeacherTimetable",
                    data: param,
                    type: "POST",
